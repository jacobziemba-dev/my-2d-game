...
export class Entity {
  constructor(x, y, width, height, color, hp, speed) {
    ...
    this.speed = speed;
    this.dead = false;
    this.immovable = false; // when true, entity resists being pushed
  }
  ...
  move(dx, dy, walls) {
    ...
    for (let wall of walls) {
      if (this.rect.colliderect(wall.rect)) {
        if (dx > 0)
          this.rect.x = wall.rect.left - this.rect.w - 1; // Add 1px buffer
    ...
    for (let wall of walls) {
      if (this.rect.colliderect(wall.rect)) {
        if (dy > 0)
          this.rect.y = wall.rect.top - this.rect.h - 1; // Add 1px buffer
  ...
  applyKnockback(vx, vy, force = 1) {
    if (this.immovable) return;
    // Simple instantaneous positional knockback (no velocity system yet)
    this.rect.x += vx * force;
    this.rect.y += vy * force;
  }
}
